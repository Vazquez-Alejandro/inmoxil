...
export async function createWorkspace(name: string, slug: string, ownerId: string): Promise<WorkspaceRow> {
  ...
  const { error: userError } = await getDb()
    .from('users').insert({ id: ownerId, workspace_id: workspace.id, role: 'owner' })
...
export async function updateWorkspaceBrand(workspaceId: string, brand: Record<string, any>): Promise<WorkspaceRow> {
  const { data, error } = await getDb()
    .from('workspaces').update(brand).eq('id', workspaceId).select().single()
  if (error) throw error
  return data
}
...
export async function checkCredits(workspaceId: string): Promise<number> {
  const { data, error } = await getDb()
    .from('workspaces').select('credits_remaining').eq('id', workspaceId).single()
  if (error) throw error
  return data.credits_remaining
}

export async function deductCredit(workspaceId: string, adId: string): Promise<boolean> {
  const { data, error } = await getDb().rpc('deduct_credit', { p_workspace_id: workspaceId, p_ad_id: adId })
  if (error) throw error
  return data
}

export async function addCredits(workspaceId: string, amount: number, description: string): Promise<boolean> {
  const { data, error } = await getDb().rpc('add_credits', { p_workspace_id: workspaceId, p_amount: amount, p_description: description })
  if (error) throw error
  return data
}
...
export async function getCreditHistory(workspaceId: string, limit = 50) {
  const { data, error } = await getDb()
    .from('credit_transactions').select('*').eq('workspace_id', workspaceId)
...
export async function setStripeIds(workspaceId: string, customerId: string, subscriptionId: string) {
  const { error } = await getDb()
    .from('workspaces').update({ stripe_customer_id: customerId, stripe_subscription_id: subscriptionId })
    .eq('id', workspaceId)
  if (error) throw error
}
