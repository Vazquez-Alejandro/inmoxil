...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, brand: { name: workspace.name, logo_url: workspace.logo_url, primary_color: workspace.primary_color, secondary_color: workspace.secondary_color, accent_color: workspace.accent_color } })
  } catch {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const workspace = await updateWorkspaceBrand(workspaceId, { logo_url, primary_color, secondary_color, accent_color })
    if (!workspace) return NextResponse.json({ error: 'Workspace no encontrado' }, { status: 404 })
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!workspaceId || !file) return NextResponse.json({ error: 'Se requiere workspaceId y file' }, { status: 400 })

    const { error } = await requireWorkspaceAuth(workspaceId)
    if (error) return error

    if (typeof file !== 'string' || file.length > 5_000_000) {
