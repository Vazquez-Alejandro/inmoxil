...
export async function POST(request: Request) {
  try {
    ...
    if (!email || !password || !name || !companyName) return NextResponse.json({ error: 'Todos los campos son requeridos' }, { status: 400 })
    if (password.length < 8) return NextResponse.json({ error: 'La contraseña debe tener al menos 8 caracteres' }, { status: 400 })
    if (!/[A-Z]/.test(password) || !/[0-9]/.test(password)) return NextResponse.json({ error: 'La contraseña debe tener al menos 1 mayúscula y 1 número' }, { status: 400 })
